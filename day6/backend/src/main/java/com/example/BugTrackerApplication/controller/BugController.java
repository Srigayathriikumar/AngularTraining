...
public class BugController {
    ...
    @GetMapping
    public ResponseEntity<java.util.Map<String, Object>> getAllBugs() {
        List<BugResponseDTO> bugs = bugService.getAllBugs();
        java.util.Map<String, Object> response = new java.util.HashMap<>();
        response.put("content", bugs);
        response.put("totalElements", bugs.size());
        return ResponseEntity.ok(response);
    }

    // ADMIN only - Create bug
    @PostMapping("/admin")
